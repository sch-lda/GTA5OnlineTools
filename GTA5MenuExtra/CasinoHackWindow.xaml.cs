    ...
    private void CasinoHackLoopThread()
    {
        while (!_disposed)
        {
            ...
            if (Memory.IsValid(pScript))
            {
                ...
                var index = Memory.Read<int>(pointer + (2033 + 2 + 1 + 1 * 1) * 8);
                CasinoHackModel.BlackjackContent = GetBlackJackContent(index);

                ///////////// 下一张牌 /////////////

                var current_table = Memory.Read<int>(pointer + (1776 + 1 + Globals.GetPlayerID() * 8 + 4) * 8);
                var nums = Memory.Read<int>(pointer + (116 + 1 + 1 + current_table * 211 + 209) * 8);

                index = Memory.Read<int>(pointer + (2033 + 2 + 1 + nums * 1) * 8);
            ...
            if (Memory.IsValid(pScript))
            {
                ...
                var index = Memory.Read<int>(pointer + (1038 + 799 + 2 + 1 + 2 * 1) * 8);
                CasinoHackModel.Poker1Content = GetBlackJackContent(index);

                index = Memory.Read<int>(pointer + (1038 + 799 + 2 + 1 + 0 * 1) * 8);
                CasinoHackModel.Poker2Content = GetBlackJackContent(index);

                index = Memory.Read<int>(pointer + (1038 + 799 + 2 + 1 + 1 * 1) * 8);
            ...
            if (Memory.IsValid(pScript))
            {
                ...
                for (var i = 0; i < 6; i++)
                {
                    Memory.Write(pointer + (124 + 1357 + 153 + 1 + i * 1) * 8, rouletteSlot);
                }
            }
